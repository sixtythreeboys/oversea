...
import { getDateDiff, getDateList } from 'src/common/util/dateUtils';
...
export async function updateByRow({ excd, symb, xymd, clos, rate }) {
  ...
  if (
    (lastData === undefined && rate === 0) ||
    (lastData !== undefined && lastData.xymd === xymd)
  ) {
    return false;
  } else if (lastData.prdyCtrt * rate < 0) {
